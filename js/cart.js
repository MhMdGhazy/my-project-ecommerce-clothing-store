document.addEventListener("DOMContentLoaded", () => {
  ...
  if (cart.length === 0) {
    ...
    cartTotal.textContent = "Total: 0.00 EG";
  ...
  function renderCart() {
    ...
    cart.forEach((product, index) => {
      ...
      row.innerHTML = `
        ...
        <td class="bg-secondary-subtle">
          <h4 class="text-center mt-5">${product.price.toFixed(2)} EG </h4>
        </td>
        ...
        <td class="bg-secondary-subtle">
          <h4 class="text-center mt-5">${itemTotal.toFixed(2)} EG</h4>
        </td>
        <td class="text-center bg-light">
          <div
    ...
    cartTotal.textContent = `Total: ${total.toFixed(2)} EG`;
